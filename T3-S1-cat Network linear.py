...
def eval_linear(args):
    ...
    if args.arch in vits.__dict__.keys():
        ...
        model3 = vits.__dict__[args.arch](patch_size=args.patch_size, num_classes=0)
        embed_dim = model1.embed_dim * (args.n_last_blocks + int(args.avgpool_patchtokens))
    # if the network is a XCiT
    elif "xcit" in args.arch:
        model1 = torch.hub.load('facebookresearch/xcit:main', args.arch, num_classes=0)
        model2 = torch.hub.load('facebookresearch/xcit:main', args.arch, num_classes=0)
        model3 = torch.hub.load('facebookresearch/xcit:main', args.arch, num_classes=0)
        embed_dim = model1.embed_dim
    # otherwise, we check if the architecture is in torchvision models
    elif args.arch in torchvision_models.__dict__.keys():
        model1 = torchvision_models.__dict__[args.arch]()
        model2 = torchvision_models.__dict__[args.arch]()
        model3 = torchvision_models.__dict__[args.arch]()
    ...
    utils.load_pretrained_weights(model2, args.pretrained_weights, args.checkpoint_key2, args.arch,
                                  args.patch_size)
    utils.load_pretrained_weights(model3, args.pretrained_weights, args.checkpoint_key3, args.arch,
                                  args.patch_size)
    ...
    if args.evaluate:
        ...
        test_stats = validate_network(val_loader, model1, model2, model3, linear_classifier, args.n_last_blocks,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train(model1, model2, model3, linear_classifier, optimizer,
...
def train(model1, model2, model3, linear_classifier, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        with torch.no_grad():
            if "vit" in args.arch:
                intermediate_output1 = model1.get_intermediate_layers(inp, n)
                intermediate_output2 = model2.get_intermediate_layers(inp, n)
                intermediate_output3 = model3.get_intermediate_layers(inp, n)
                output1 = torch.cat([x[:, 0] for x in intermediate_output1], dim=-1)
                output2 = torch.cat([x[:, 0] for x in intermediate_output2], dim=-1)
                output3 = torch.cat([x[:, 0] for x in intermediate_output3], dim=-1)
                if avgpool:
                    output1 = torch.cat(
...
class LinearClassifier(nn.Module):
    ...
    def __init__(self, dim, num_labels):
        ...
        self.linear = nn.Linear(int(dim * 3), num_labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_weights', default='./T3-S1 Network/checkpoint.pth', type=str,
    ...
    parser.add_argument("--checkpoint_key2", default="teacher2", type=str,
                        help='Key to use in the checkpoint (example: "teacher")')
    parser.add_argument("--checkpoint_key3", default="teacher3", type=str,
                        help='Key to use in the checkpoint (example: "teacher")')
